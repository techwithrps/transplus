...
export const generateVehicleWiseGR = (request, vehicleData) => {
  console.debug("generateVehicleWiseGR called", {
  ...
  // Helper function to get container details from vehicle data
  const getContainerDetails = () => {
    if (!vehicleData) return null;

    // If containers array exists, get first container
    if (vehicleData.containers && vehicleData.containers.length > 0) {
      return vehicleData.containers[0];
    }

    // Use direct properties from vehicle data
    return {
      container_no: vehicleData.container_no,
      line: vehicleData.line,
      seal_no: vehicleData.seal_no,
    };
  };
  ...
  copies.forEach((copyType, index) => {
    ...
    // Use vehicle-specific data with fallbacks
    const vehicleNumber = vehicleData?.vehicle_number || "Not Assigned";
    const driverName = vehicleData?.driver_name || "Not Assigned";
    const driverContact = vehicleData?.driver_contact || "Not Available";
    ...
    // Second section with container details using vehicle-specific API data
    const getVehicleSize = () => {
      // First priority: vehicle-specific size
      if (vehicleData?.vehicle_size) {
        const match = vehicleData.vehicle_size.match(/(\d+)/);
        return match ? match[1] : "40";
      }

      // Second priority: container details size
      if (containerDetails?.size) {
        return containerDetails.size;
      }

      // Third priority: request vehicle size
      if (request.vehicle_size) {
        const match = request.vehicle_size.match(/(\d+)/);
        return match ? match[1] : "40";
      }

      // Fourth priority: determine from container counts
    ...
    // Get properly aligned container details from vehicle data
    const getContainerNumber = () => {
      return (
        containerDetails?.container_no ||
        vehicleData?.container_no ||
        vehicleData?.container_number ||
    ...
    const getSealNumber = () => {
      return (
        containerDetails?.seal_no ||
        vehicleData?.seal_no ||
        vehicleData?.seal_number ||
        request.seal_number ||
        "Not Available"
      );
    };
    ...
    // Footer with dynamic URL using API data and vehicle number
    doc.setFontSize(6);
    doc.setTextColor(0, 0, 255);
    const grNumber =
      request.formatted_request_id || request.id?.toString() || "default";
    const vehicleRef =
      vehicleData?.vehicle_number?.replace(/\s+/g, "") || "vehicle";
    doc.text(
      `https://www.elogfreight.com/GR.PH?uen6PRNT&Pthm-num-IDNO=sp=${grNumber}-${vehicleRef}`,
      14,
      285
    );
    doc.setTextColor(0, 0, 0);
...
// export default for simpler imports (keeps named export for backward-compat)
export default generateVehicleWiseGR;
